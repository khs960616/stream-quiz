...
public class Quiz5 {
    ...
    public int quiz1() {
        return Arrays.stream(STRING_ARR).mapToInt(String::length).sum();
    }

    // 5.2 문자열 중에서 가장 긴 것의 길이를 출력하시오.
    public int quiz2() {
        return Arrays.stream(STRING_ARR).mapToInt(String::length).max().orElse(0);
    }
    ...
    public List<Integer[]> quiz4() {
        return IntStream.rangeClosed(1, 6)
            .boxed()
            .flatMap(n->IntStream.rangeClosed(1, 6).filter(m->n+m==6).boxed().map(m->new Integer[]{n, m}))
            .collect(Collectors.toList());
    }
